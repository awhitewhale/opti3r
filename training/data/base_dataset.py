...
class BaseDataset(Dataset):
    ...
    def process_two_image(
        ...
        """
        # Make copies to avoid in-place operations affecting original data
        no_water_image = np.copy(no_water_image)
        water_image = np.copy(water_image)
        depth_map = np.copy(depth_map)
        extri_opencv = np.copy(extri_opencv)
        intri_opencv = np.copy(intri_opencv)
        if track is not None:
        ...
        # Move principal point to the image center and crop if necessary
        no_water_image, water_image, depth_map, intri_opencv, track = crop_two_image_depth_and_intrinsic_by_pp(
            no_water_image, water_image, depth_map, intri_opencv, aug_size, track=track, filepath=filepath,
        )

        original_size = np.array(water_image.shape[:2])  # update original_size
        target_shape = target_image_shape
        ...
        # Resize images and update intrinsics
        if self.rescale:
            no_water_image, water_image, depth_map, intri_opencv, track = resize_two_image_depth_and_intrinsic(
                no_water_image, water_image, depth_map, intri_opencv, target_shape, original_size, track=track,
                safe_bound=safe_bound,
                rescale_aug=self.rescale_aug
            )
        else:
        ...
        # Ensure final crop to target shape
        no_water_image, water_image, depth_map, intri_opencv, track = crop_two_image_depth_and_intrinsic_by_pp(
            no_water_image, water_image, depth_map, intri_opencv, target_shape, track=track, filepath=filepath,
            strict=True,
        )
        ...
        if rotate_to_portrait:
            assert self.landscape_check
            clockwise = np.random.rand() > 0.5
            no_water_image, water_image, depth_map, extri_opencv, intri_opencv, track = rotate_90_degrees_for_two(
                no_water_image,
                water_image,
                depth_map,
                extri_opencv,
    ...
    def get_nearby_ids(self, ids, full_seq_num, expand_ratio=None, expand_range=None):
        """
        NOTE: add the function to sample the ids by pose similarity ranking.
