...
def crop_image_depth_and_intrinsic_by_pp(
    ...
    """
    NOTE: some names of width and height seem not consistent. Need to check.
...
######################## two
def crop_two_image_depth_and_intrinsic_by_pp(
        no_water_image, water_image, depth_map, intrinsic, target_shape, track=None, filepath=None, strict=False
):
    """
    NOTE: some names of width and height seem not consistent. Need to check.
    ...
    Returns:
        tuple:
            ...
            - updated_track (np.ndarray or None): Track array adjusted for the crop, or None if track was not provided.
    """
    original_size = np.array(no_water_image.shape)
    intrinsic = np.copy(intrinsic)
    ...
    # Perform the crop
    no_water_image = no_water_image[start_x:end_x, start_y:end_y, :]
    water_image = water_image[start_x:end_x, start_y:end_y, :]
    if depth_map is not None:
        depth_map = depth_map[start_x:end_x, start_y:end_y]
    ...
    # If strict, zero-pad if the new shape is smaller than target_shape
    if strict:
        if (water_image.shape[:2] != target_shape).any():
            print(f"{filepath} does not meet the target shape")
            current_h, current_w = water_image.shape[:2]
            target_h, target_w = target_shape[0], target_shape[1]
            pad_h = target_h - current_h
            pad_w = target_w - current_w
            if pad_h < 0 or pad_w < 0:
    ...
    return no_water_image, water_image, depth_map, intrinsic, track

def rotate_image_and_depth_rot90_for_two(no_water_image, water_image, depth_map, clockwise):
    """
    Rotates the given image and depth map by 90 degrees (clockwise or counterclockwise),
    using a transpose+flip pattern.
    ...
    rotated_depth_map = None
    if clockwise:
        rotated_no_water_image = np.transpose(no_water_image, (1, 0, 2))  # Transpose height and width
        rotated_no_water_image = np.flip(rotated_no_water_image, axis=1)  # Flip horizontally
        rotated_water_image = np.transpose(water_image, (1, 0, 2))  # Transpose height and width
        rotated_water_image = np.flip(rotated_water_image, axis=1)  # Flip horizontally
        if depth_map is not None:
            rotated_depth_map = np.transpose(depth_map, (1, 0))
            rotated_depth_map = np.flip(rotated_depth_map, axis=1)
    else:
        rotated_no_water_image = np.transpose(no_water_image, (1, 0, 2))  # Transpose height and width
        rotated_no_water_image = np.flip(rotated_no_water_image, axis=0)  # Flip vertically
        rotated_water_image = np.transpose(water_image, (1, 0, 2))  # Transpose height and width
        rotated_water_image = np.flip(rotated_water_image, axis=0)  # Flip vertically
        if depth_map is not None:
            rotated_depth_map = np.transpose(depth_map, (1, 0))
            rotated_depth_map = np.flip(rotated_depth_map, axis=0)
    return np.copy(rotated_no_water_image), np.copy(rotated_water_image), np.copy(rotated_depth_map)


def rotate_90_degrees_for_two(
        no_water_image, water_image, depth_map, extri_opencv, intri_opencv, clockwise=True, track=None
):
    """
    Rotates the input image, depth map, and camera parameters by 90 degrees.
    ...
    Returns:
        tuple:
            ...
            Where each is the updated version after the rotation.
    """
    image_height, image_width = water_image.shape[:2]

    # Rotate the image and depth map
    rotated_no_water_image, rotated_water_image, rotated_depth_map = rotate_image_and_depth_rot90_for_two(
        no_water_image, water_image, depth_map, clockwise)
    # Adjust the intrinsic matrix
    new_intri_opencv = adjust_intrinsic_matrix_rot90(intri_opencv, image_width, image_height, clockwise)
...
def resize_two_image_depth_and_intrinsic(
        no_water_image,
        water_image,
        depth_map,
        intrinsic,
    ...
    # Convert image to PIL for resizing
    # no_water_image = Image.fromarray(no_water_image)
    # water_image = Image.fromarray(water_image)
    H, W = water_image.shape[:2]
    # input_resolution = np.array(water_image.size)
    input_resolution = np.array([W, H], dtype=np.int32)
    output_resolution = np.floor(input_resolution * max_resize_scale).astype(np.int32)
    out_w, out_h = int(output_resolution[0]), int(output_resolution[1])
    interp = cv2.INTER_LANCZOS4 if max_resize_scale < 1 else cv2.INTER_CUBIC

    # water_image = water_image.resize(tuple(output_resolution), resample=lanczos if max_resize_scale < 1 else bicubic)
    # no_water_image = no_water_image.resize(tuple(output_resolution),
    #                                        resample=lanczos if max_resize_scale < 1 else bicubic)
    def _resize_keep_1ch(img: np.ndarray, out_w: int, out_h: int, interp: int) -> np.ndarray:
        """cv2.resize  (H,W,1) """
        if img.ndim == 3 and img.shape[2] == 1:
            resized2d = cv2.resize(img[:, :, 0], (out_w, out_h), interpolation=interp)
            return resized2d[:, :, None]  #  (H,W,1)
        else:
            return cv2.resize(img, (out_w, out_h), interpolation=interp)

    water_image = _resize_keep_1ch(water_image, out_w, out_h, interp)
    no_water_image = _resize_keep_1ch(no_water_image, out_w, out_h, interp)
    #
    # no_water_image = np.array(no_water_image)
    # water_image = np.array(water_image)
    ...
    actual_size = np.array(water_image.shape[:2])
    actual_resize_scale = np.max(actual_size / original_size)
    ...
    assert water_image.shape[:2] == depth_map.shape[:2]
    return no_water_image, water_image, depth_map, intrinsic, track