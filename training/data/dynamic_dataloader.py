...
class DynamicTorchDataset(ABC):
    def __init__(
        ...
        # self.no_label_data_probability = common_config.no_label_data_probability  # e.g.,0.5
        # self.only_depth_label_dataset_probability = common_config.only_depth_label_dataset_probability
        self.no_label_data_probability = 0.  # e.g.,0.5
        self.only_depth_label_dataset_probability = 0.
        ...
        self.datasets_has_no_label_dataset = False
        no_label_dataset_config = {
            'datasets_has_no_label_dataset': False
        }
        self.datasets_has_only_depth_label_dataset = False
        only_depth_label_dataset_config = {
            'datasets_has_only_depth_label_dataset': False
        }

        for i, ds in enumerate(self.dataset.base_dataset.datasets):
            if hasattr(ds, "no_label_dataset"):
                self.no_label_data_probability = common_config.no_label_data_probability  # e.g.,0.5
                assert ds.no_label_dataset is True, f'{ds} no_label_dataset is not init correct'
                assert 0 < self.no_label_data_probability < 1, \
                    f"no_label_data_probability must be in (0,1), got {self.no_label_data_probability}"
                # assert i == 0, print(f'lazy code, please put no label dataset in first place, but got {i + 1} place')
                no_label_dataset_config = {
                    'datasets_has_no_label_dataset': True,
                    'number_of_no_label_dataset': i,
                    'nums_no_label_dataset': len(ds),
                    'no_label_data_probability': self.no_label_data_probability,
                }

            if hasattr(ds, "only_depth_label_dataset"):
                self.only_depth_label_dataset_probability = common_config.only_depth_label_dataset_probability

                assert ds.only_depth_label_dataset is True, f'{ds} only_depth_label_dataset is not init correct'
                assert 0 < self.only_depth_label_dataset_probability < 1, \
                    f"only_depth_label_dataset_probability must be in (0,1), got {self.only_depth_label_dataset_probability}"
                # assert i == 1, print(f'lazy code, please put no label dataset in first place, but got {i + 1} place')

                only_depth_label_dataset_config = {
                    'datasets_has_only_depth_label_dataset': True,
                    'number_of_only_depth_label_dataset': i,
                    'nums_only_depth_label_dataset': len(ds),
                    'only_depth_label_dataset_probability': self.only_depth_label_dataset_probability,
                }
        if not dist.is_initialized() or dist.get_rank() == 0:
            logging.info(f'no_label_dataset_config: {no_label_dataset_config}')
            logging.info(f'only_depth_label_dataset_config: {only_depth_label_dataset_config}')
        self.batch_sampler = DynamicBatchSampler(
            ...
            no_label_dataset_config=no_label_dataset_config,
            only_depth_label_dataset_config=only_depth_label_dataset_config,
        )
...
class DynamicBatchSampler(Sampler):
    ...
    def __init__(self,
                 ...
                 seed=42,
                 max_img_per_gpu=48,
                 no_label_dataset_config=None,
                 only_depth_label_dataset_config=None,
                 ):
        """
        ...
        ########################################
        self.no_label_dataset_config = no_label_dataset_config
        self.only_depth_label_dataset_config = only_depth_label_dataset_config
        self.cfg_no = no_label_dataset_config or {'datasets_has_no_label_dataset': False}
        self.cfg_only = only_depth_label_dataset_config or {'datasets_has_only_depth_label_dataset': False}
        self.p_no = float(self.cfg_no.get('no_label_data_probability', 0.0)) \
            if self.cfg_no.get('datasets_has_no_label_dataset', False) else 0.0
        self.p_only = float(self.cfg_only.get('only_depth_label_dataset_probability', 0.0)) \
            if self.cfg_only.get('datasets_has_only_depth_label_dataset', False) else 0.0

        if self.p_no + self.p_only > 1.0:
            raise ValueError(
                f"no-label and only-depth probabilities must sum to <= 1, got {self.p_no + self.p_only}"
            )
        self.p_norm = max(0.0, 1.0 - self.p_no - self.p_only)
        self.use_no_label = self.p_no > 0.0 or self.p_only > 0.0
        logging.info(f'\nno-label data probability {self.p_no}, \n'
                     f'only-depth data probability {self.p_only},\n'
                     f'normal data probability {self.p_norm}\n'
                     )
        ########################################
        # print('\n\n\n\n\n\nn_first', n_first, '\n\n\n\n\n')
    ...
    def _decide_group_tag(self) -> str:
        """Sample one dataset group tag for the next dynamic batch."""
        has_no = self.cfg_no.get('datasets_has_no_label_dataset', False)
        has_only = self.cfg_only.get('datasets_has_only_depth_label_dataset', False)

        if not has_no and not has_only:
            return 'normal'

        u = self.rng.random()
        if has_no and u < self.p_no:
            return 'no_label'
        if has_only and u < self.p_no + self.p_only:
            return 'only_depth'
        if self.p_norm > 0.0:
            return 'normal'
        return 'no_label' if has_no else 'only_depth'
    ...
    def __iter__(self):
        ...
        while True:
            try:
                ...
                current_batch = []
                if self.use_no_label:
                    group_tag = self._decide_group_tag()
                else:
                    group_tag = 'normal'
