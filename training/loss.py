...
from wat3r.utils.pose_enc import extri_intri_to_pose_encoding
from training.optir3r import intervention_consistency_loss
...
class MultitaskLoss(torch.nn.Module):
    ...
    def __init__(self, camera=None, depth=None, point=None, track=None, water=None, kd=None, ema=None, device=None,
                 **kwargs):
        super().__init__()
        ...
        self.point = point
        self.ema = ema
        self.intervention = kwargs.get("intervention", None) or {}
        self.swap = kwargs.get("swap", None) or {}
        self.optics = kwargs.get("optics", None) or {}
        self.static_loss = kwargs.get("static_loss", "teacher")
        if self.static_loss != "teacher":
            raise NotImplementedError("Only static_loss='teacher' is kept in the cleaned Wat3R training code.")
        # ema_type = self.ema['type']
        self.ema_weight = float(self.ema.get('weight', 0.5))
        from training.losses.ema_loss import (
            ema_compute_camera_loss,
            ema_compute_depth_loss,
            ema_compute_point_loss,
        )
        from training.losses.static_geo_loss import static_geometry_loss
        self.ema_compute_depth_loss = ema_compute_depth_loss
        self.ema_compute_point_loss = ema_compute_point_loss
        self.ema_compute_camera_loss = ema_compute_camera_loss
        self.static_geometry_loss = static_geometry_loss

        self.device = device

    def forward(self, predictions, batch, teacher_out=None, t_temp=1., counterfactual_predictions=None, swap_predictions=None):
        """
        ...
        ema_total_loss = 0
        loss_dict = {}
        # Keep every optics sub-head in the autograd graph even for ablations
        # that disable its supervised term.  This lets the normal static DDP
        # reducer handle the matrix, including the swap double-forward path.
        if "optics_params" in predictions:
            total_loss = total_loss + 0.0 * (
                predictions["optics_params"].sum() + predictions["optics_quality"].sum()
            )
        if teacher_out is None:
            ...
            batch.pop('depths')
            batch.pop('extrinsics')
            batch.pop('intrinsics')
            ...
            if "optics_params" in predictions and "optics_targets" in batch and float(self.optics.get("weight", 0.0)) > 0:
                optics_loss = F.smooth_l1_loss(predictions["optics_params"], batch["optics_targets"].to(predictions["optics_params"].dtype))
                loss_dict["loss_optics"] = optics_loss
                total_loss = total_loss + float(self.optics["weight"]) * optics_loss

            if counterfactual_predictions is not None and float(self.intervention.get("weight", 0.0)) > 0:
                intervention_dict = intervention_consistency_loss(predictions, counterfactual_predictions)
                loss_dict.update(intervention_dict)
                total_loss = total_loss + float(self.intervention["weight"]) * intervention_dict["loss_intervene"]
            if swap_predictions is not None and float(self.swap.get("weight", 0.0)) > 0:
                if "optics_embedding" not in predictions or "optics_embedding" not in swap_predictions:
                    raise KeyError("swap consistency requires optics_embedding predictions")
                swap_dict = {
                    "loss_swap_embedding": F.smooth_l1_loss(
                        predictions["optics_embedding"], swap_predictions["optics_embedding"]
                    )
                }
                loss_dict.update(swap_dict)
                total_loss = total_loss + float(self.swap["weight"]) * swap_dict["loss_swap_embedding"]
        else:
            t_temp = torch.tensor(t_temp, device=self.device)
            t = torch.clamp(t_temp, 0.0, 1.0)
            ema_lambda = t * self.ema_weight

            if "pose_enc_list" in predictions:
                camera_loss_dict = self.ema_compute_camera_loss(predictions, teacher_out, **self.camera)
                camera_loss = camera_loss_dict["ema_loss_camera"] * self.camera["weight"]
                ema_total_loss = ema_total_loss + camera_loss
                loss_dict.update(camera_loss_dict)

            if "depth" in predictions:
                depth_loss_dict = self.ema_compute_depth_loss(predictions, teacher_out, **self.depth)
                depth_loss = depth_loss_dict["ema_loss_conf_depth"] + depth_loss_dict["ema_loss_reg_depth"] + \
                             depth_loss_dict[
                                 "ema_loss_grad_depth"]
                depth_loss = depth_loss * self.depth["weight"]
                ema_total_loss = ema_total_loss + depth_loss
                loss_dict.update(depth_loss_dict)

            if "world_points" in predictions:
                point_loss_dict = self.ema_compute_point_loss(predictions, teacher_out, **self.point)
                point_loss = point_loss_dict["ema_loss_conf_point"] + point_loss_dict["ema_loss_reg_point"] + \
                             point_loss_dict[
                                 "ema_loss_grad_point"]
                point_loss = point_loss * self.point["weight"]
                ema_total_loss = ema_total_loss + point_loss
                loss_dict.update(point_loss_dict)
                teacher_out.pop('point_masks')
                teacher_out.pop('world_points')
                torch.cuda.empty_cache()

            if 'static_masks' in teacher_out:
                evidence_weight = teacher_out.get('evidence_weights', teacher_out['static_masks'])
                static_loss_dict = self.static_geometry_loss(
                    teacher_out=teacher_out,
                    student_out=predictions,
                    static_mask=evidence_weight,
                    **self.depth,
                )
                ema_total_loss = ema_total_loss + static_loss_dict["ema_loss_static_geo"]
                loss_dict.update(static_loss_dict)
                teacher_out.pop('static_masks')
                teacher_out.pop('evidence_weights', None)

            loss_dict['ema_loss_objective'] = ema_total_loss

            total_loss = ema_total_loss * ema_lambda + total_loss
...
def compute_camera_loss(
    ...
    image_hw = batch_data['point_masks'].shape[-2:]
...
def compute_depth_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn=None, valid_range=-1, **kwargs):
    ...
    # print('pred_depth1',pred_depth.shape)
    # print('gt_depth1',gt_depth.shape)
    # print('gt_depth_mask1',gt_depth_mask.shape)
    # print('pred_depth_conf1',pred_depth_conf.shape)
