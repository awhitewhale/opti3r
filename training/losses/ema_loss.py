...
def ema_compute_camera_loss(
    ...
    image_hw = batch_data['depth'].shape[-3:-1]
    ...
    return {
        "ema_loss_camera": total_camera_loss,
        "ema_loss_T": avg_loss_T,
        "ema_loss_R": avg_loss_R,
        "ema_loss_FL": avg_loss_FL
    }
...
def ema_compute_point_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn=None, valid_range=-1, **kwargs):
    """
    Compute point loss.
    ...
    # print('gt_points',gt_points.dtype)
    ...
    if gt_points_mask.sum() < 100:
        ...
        loss_dict = {f"ema_loss_conf_point": dummy_loss,
                     f"ema_loss_reg_point": dummy_loss,
                     f"ema_loss_grad_point": dummy_loss, }
        return loss_dict
    ...
    loss_dict = {
        f"ema_loss_conf_point": loss_conf,
        f"ema_loss_reg_point": loss_reg,
        f"ema_loss_grad_point": loss_grad,
    }

    return loss_dict


def ema_compute_depth_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn=None, valid_range=-1, **kwargs):
    """
    Compute depth loss.
    ...
    if gt_depth_mask.sum() < 100:
        ...
        loss_dict = {f"ema_loss_conf_depth": dummy_loss,
                     f"ema_loss_reg_depth": dummy_loss,
                     f"ema_loss_grad_depth": dummy_loss, }
        return loss_dict
    ...
    loss_dict = {
        f"ema_loss_conf_depth": loss_conf,
        f"ema_loss_reg_depth": loss_reg,
        f"ema_loss_grad_depth": loss_grad,
    }
...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1):
    ...
    # Support both Wat3R's boolean masks and Opti3R's continuous evidence weights.
    mask_bool = mask.bool() if mask.dtype == torch.bool else mask > 1e-6
    mask_weight = mask[mask_bool].float() if mask.dtype != torch.bool else torch.ones_like(mask[mask_bool], dtype=pred.dtype)

    # Compute L2 distance between predicted and ground truth points.
    loss_reg = torch.norm(gt[mask_bool] - pred[mask_bool], dim=-1)
    # print('loss_reg',loss_reg.dtype)
    loss_reg = check_and_fix_inf_nan(loss_reg, "loss_reg")
    loss_reg = loss_reg * mask_weight
    ...
    loss_conf = (gamma * loss_reg * conf[mask_bool] - alpha * torch.log(conf[mask_bool])) * mask_weight
