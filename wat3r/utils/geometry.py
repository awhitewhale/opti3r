...
from wat3r.dependency.distortion import apply_distortion, iterative_undistortion, single_undistortion
...
def unproject_depth_map_to_point_map_bs(
    depth_map,          # (B,S,H,W) or (B,S,H,W,1) - torch or np
    extrinsics_cam,     # (B,S,3,4)
    ...
    Args:
        ...
        extrinsics_cam: (B,S,3,4)  cam-from-world
        intrinsics_cam: (B,S,3,3)
    ...
    # world = R * cam + t
    # (B,S,HW,3) @ (B,S,3,3)^T + (B,S,1,3)
    world_flat = torch.matmul(cam_flat, R.transpose(-1, -2)) + t.unsqueeze(2)  # (B,S,HW,3)
    world_points = world_flat.view(B, S, H, W, 3)

    return world_points, cam_points, mask


def unproject_depth_map_to_world_point(
    depth_map,          # (B,S,H,W) or (B,S,H,W,1) - torch or np
    extrinsics_cam,     # (B,S,3,4)
    intrinsics_cam,     # (B,S,3,3)
    eps: float = 1e-8,
...
def unproject_depth_map_to_cam_point_bs(
    depth_map,          # (B,S,H,W) or (B,S,H,W,1) - torch or np
    extrinsics_cam,     # (B,S,3,4)
    intrinsics_cam,     # (B,S,3,3)
    eps: float = 1e-8,
...
def depth_to_cam_coords_points_bs(
    depth_map: torch.Tensor,     # (B,S,H,W)
    ...
    """
    B, S, H, W = depth_map.shape
    assert intrinsics.shape == (B, S, 3, 3), "intrinsics must be (B,S,3,3)"
